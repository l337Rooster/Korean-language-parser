...
export default {
	...
	methods: {
        ...
        requestParse: function () {
            ...
            $.ajax({
                method: "POST",
                url: 'http://10.0.1.77:9000/parse/', // '/parse/', // 'http://localhost:9000/parse/',
                crossDomain: true,
                cache: false,
        ...
        showReferences: function (node, event) {
            ...
            if (Math.abs(self.mouseEnterX - window.screenX) < 20 && Math.abs(self.mouseEnterY - window.screenY) < 20) {
                ...
                $.ajax({
                    method: "GET",
                    url: "http://10.0.1.77:9000/definition/" + word, // "/definition/" + word, // "http://localhost:9000/definition/" + word,
                    crossDomain: true,
                    cache: false,
                    success: function (response) {
        ...
        mouseEnterWord: function (node, event) {
            ...
            self.mouseEnterX = event.screenX;
            self.mouseEnterY = event.screenY;
