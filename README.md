...
The phrase parse-tree graph that appears under the phonemes is derived in a syntax-parsing phase that analyzes the mapped phoneme & POS tag
...
library's "chunking" 
capabilities and is driven by a RegExp-based chunking grammar located in the ``backend.chunker`` module.
